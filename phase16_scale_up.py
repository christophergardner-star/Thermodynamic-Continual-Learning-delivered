"""
Phase 16 - Scale-up: Split-CIFAR-100
====================================

Import-safe native runner for the CIFAR-100 scale-up suite.
"""
...
def _log_path(workspace: str) -> Path:
    return Path(workspace) / "tar_state" / "logs" / "phase16.log"
...
def _load_hf_cifar100(workspace: str):
    import datasets as hf_datasets

    def _find_cached_arrow_pair() -> tuple[Path, Path] | None:
        candidates = [
            Path(workspace) / "dataset_artifacts" / "cifar100_hf",
            Path(_repo) / "dataset_artifacts" / "cifar100_hf",
        ]
        for base in candidates:
            train_matches = sorted(base.rglob("cifar100-train.arrow"))
            test_matches = sorted(base.rglob("cifar100-test.arrow"))
            if train_matches and test_matches:
                return train_matches[0], test_matches[0]
        return None

    arrow_pair = _find_cached_arrow_pair()
    if arrow_pair is not None:
        train_arrow, test_arrow = arrow_pair
        print("  Loading CIFAR-100 from cached Arrow files...", flush=True)
        train_ds = hf_datasets.Dataset.from_file(str(train_arrow))
        test_ds = hf_datasets.Dataset.from_file(str(test_arrow))
        sample = train_ds[0]
        img_key = "img" if "img" in sample else "image"
        label_key = "fine_label" if "fine_label" in sample else "label"
        train_items = [(row[img_key], int(row[label_key])) for row in train_ds]
        test_items = [(row[img_key], int(row[label_key])) for row in test_ds]
        print(f"  Loaded {len(train_items)} train / {len(test_items)} test samples", flush=True)
        return train_items, test_items

    cache_dir = Path(workspace) / "dataset_artifacts" / "cifar100_hf"
    cache_dir.mkdir(parents=True, exist_ok=True)
    print("  Loading CIFAR-100 from HuggingFace (downloads on first run)...", flush=True)
    ds = hf_datasets.load_dataset("uoft-cs/cifar100", cache_dir=str(cache_dir))
    sample = ds["train"][0]
    img_key = "img" if "img" in sample else "image"
    label_key = "fine_label" if "fine_label" in sample else "label"
    train_items = [(row[img_key], int(row[label_key])) for row in ds["train"]]
    test_items = [(row[img_key], int(row[label_key])) for row in ds["test"]]
    print(f"  Loaded {len(train_items)} train / {len(test_items)} test samples", flush=True)
    return train_items, test_items
...
def _build_cifar100_tasks(seed: int, train_items: list, test_items: list):
    ...
    for task_classes in class_order:
        ...
        tr = [(img, label) for img, label in train_items if label in task_set]
        te = [(img, label) for img, label in test_items if label in task_set]
...
def run_one_seed(
    ...
    for train_t in range(N_TASKS):
        ...
        optimizer = build_optimizer(
            ...
            momentum=0.9,
            workspace=workspace,
            run_label=f"phase16-cifar100-{method}-seed{seed}-task{train_t}",
        ...
        if method == "ewc":
            ...
            for bx, by in sample_loader:
                ...
                bx, by = bx.to(device), by.to(device)
                reps = trunk(bx)
                log_probs = F.log_softmax(heads[train_t](reps), dim=1)
...
def _run_phase16_suite_impl(
    ...
    print(f"\n{'='*70}")
    print("Phase 16 - Scale-up: Split-CIFAR-100 (native runner)")
    ...
    try:
        with tracked_process(
            ...
            train_items, test_items = _load_hf_cifar100(workspace)
    ...
    if vs_ewc["mean_delta"] < -0.01 and vs_ewc["p_val"] < 0.05 and vs_ewc["cohens_d"] > 0.5:
        ...
        verdict = (
            f"OUTCOME A - TCL beats EWC on CIFAR-100 (delta={vs_ewc['mean_delta']:+.4f}, "
        ...
        verdict = (
            f"OUTCOME B - TCL beats SGD on CIFAR-100 (delta={vs_sgd['mean_delta']:+.4f}, "
            f"p={vs_sgd['p_val']:.4f}, d={vs_sgd['cohens_d']:.2f}). Generalises beyond CIFAR-10."
        )
    else:
        verdict_key = "OUTCOME_C"
        verdict = (
            "OUTCOME C - No significant improvement on CIFAR-100. TCL does not generalise "
...
def run_phase16_suite(
    ...
    with tee_console(_log_path(workspace)):
        return _run_phase16_suite_impl(
            workspace,
