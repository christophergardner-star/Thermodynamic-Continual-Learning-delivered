"""
Phase 16 CIFAR-100 Rerun — 5-seed, 7-method replacement
=========================================================

This script replaces phase16_scale_up.py (old: 3 seeds, 3 methods, no
...
--------------------------------------------------
1. E:\\TAR\\...\\tar_state\\preregistrations\\phase16_rerun.json must exist.
...
Usage
-----
    python phase16_cifar100_rerun.py

The script writes a single JSON result to:
    E:\\TAR\\...\\tar_state\\comparisons\\phase16_cifar100_rerun_<timestamp>.json
...
N_TASKS  = 10           # split_cifar100: 10 tasks × 10 classes = 100 classes
EPOCHS   = 40
...
METHODS_CONFIG: dict[str, dict] = {
    ...
    # commit 33ad4a6): this IS the canonical gradient-energy TCLMethod this
    # script always ran; only the lookup key changed.
    "tcl_canonical": {
...
RESULT_ID    = f"phase16_cifar100_rerun_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
PREREG_FILE  = Path(
    r"E:\TAR\Thermodynamic-Continual-Learning-delivered\tar_state\preregistrations\phase16_rerun.json"
)
EXEC_FLAG    = Path(
...
def main() -> None:
    ...
    print(f"\n{'='*72}")
    print(f"Phase 16 CIFAR-100 Rerun — PhD Rehabilitation Plan Phase 2")
